...
class news_data_manipulator:
    ...
    def get_tag_news_info(self, start_time_str, end_time_str):
        ...
        for item in select_query_result:
            content_id = item[0]
            top_word_per_news = [x[0] for x in json.loads(item[1]).get('data')]
            title = item[2]
    ...
    def get_event_news_info(self, event_id):
        statement = '''
              SELECT
        			fni.publish_time, fni.content_id, fni.tf_content, fni.top_word_per_news, npr.segged_title, nbr.title, nbr.content, fni.repost_num, nv.word2vec, nb.source_id, nb.platform
        		FROM
        ...
        for item in select_query_result:
            publish_time = item[0]
            content_id = item[1]
            tf_per_news = json.loads(item[2])
            top_word_per_news = [x[0] for x in json.loads(item[3]).get('data')]
            segged_title = [x[0] for x in json.loads(item[4]).get('data')]
            title = item[5]
            content = item[6]
            repost_num = item[7]
    ...
    def get_single_news_info(self, content_id):
        statement = '''
            SELECT
                a.publish_time, a.content_id, a.tf_content, a.top_word_per_news, b.segged_title, r.title, r.content, a.repost_num, nv.word2vec, nb.source_id, nb.platform
            FROM
                melonfield.filtered_news_info a
            JOIN
        ...
        for item in select_query_result:
            ...
            tf_per_news = json.loads(item[2])
            top_word_per_news = [x[0] for x in json.loads(item[3]).get('data')]
            segged_title = [x[0] for x in json.loads(item[4]).get('data') if (x[1].startswith('n') or x[1].startswith('v')) and len(x[0])>1]
            title = item[5]
            content = item[6]
            repost_num = item[7]
            word2vec = item[8].split(',')
