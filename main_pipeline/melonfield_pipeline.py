...
from melon_module.event_timeline import event_timeline
from melon_module.abstract import abstract
from melon_module.tag import tag
from melon_module.recent_hot_news import recent_hot_news
...
class main_panel():
    ...
    def event_update_timeline(self, event_info, news_pool):
        ...
        for event_id, event_dict in event_info.items():
            ...
            if len(timeline_potential_news)>0:
                history_timeline = event_info[event_id].get('history_timeline')
                human_dict = dict()
                news_collection = event_info[event_id].get('news_id_list')
                updated_timeline = self.et.update_timeline(event_info[event_id], history_timeline, timeline_potential_news, human_dict, news_collection, news_pool)
...
#         create news_pool waiting for tag
        ...
        event_cutoff_ts = (time_now - timedelta(days = 15)).strftime('%Y%m%d%H%M')
        timeline_potential_news_ts = ((time_now - timedelta(hours = 6)).strftime('%Y%m%d%H%M'),(time_now - timedelta(hours = 5.5)).strftime('%Y%m%d%H%M'))
        
        event_info = self.event_dm.get_all_event_info(event_cutoff_ts, timeline_potential_news_ts)
        ...
        # make latest news module
        event_latest_news, delete_info = self.latest_md.form_latest_module(event_updated_news, event_info, important_event_news_pool)
        self.news_dm.delete_from_news_module(delete_info)
        self.news_dm.insert_latest_news_into_news_module(event_latest_news)
        
        news_abstract_list = self.abt.get_event_latest_valid_news_abstract(event_latest_news, event_updated_news, important_event_news_pool)
        self.news_dm.insert_news_abstract(news_abstract_list)

        timeline_updated_event_info, event_timeline_insert = self.event_update_timeline(event_info, important_event_news_pool)        
