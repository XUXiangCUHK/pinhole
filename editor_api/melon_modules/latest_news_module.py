...
from models.news_simi_module import news_simi_module
from models.display_filter_module import display_filter_module
...
class latest_module:
    ...
    def event_generate_latest_news(self, event_id, news_pool):
        event_latest_news = list()
        delete_info = list()
        event_related_news = sorted(news_pool.keys(),key=lambda x:news_pool[x]['publish_time'],reverse=True)
        valid_latest_news = self.display_filter_mod.check_valid_display_news(event_related_news, news_pool, dict())
        if valid_latest_news!=list():
            valid_latest_news = self.news_simi_mod.duplicate_filter(valid_latest_news, valid_latest_news[:15], news_pool)
        if valid_latest_news!=list():
            delete_info.append((event_id, self.latest_module_type))
