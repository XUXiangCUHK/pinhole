...
@author: user
"""
from models.database_new import database
...
class news_data_manipulator:
    ...
    def get_event_related_news_data_for_timeline(self, event_id):
        statement =  '''
        SELECT 
            nbr.publish_id,nbr.title,nbr.publish_time,fni.repost_num,tf_per_news,top_word_per_news,word2vec, nbr.content, nb.source_id
        FROM
            melonfield.event_related_news ern
        		JOIN
        	streaming.news_basics_raw nbr on ern.content_id = nbr.publish_id
             JOIN
          streaming.news_basics nb on ern.content_id = nb.content_id
        		JOIN
        	melonfield.filtered_news_info fni on ern.content_id = fni.content_id
        		JOIN
        	streaming.news_word2vec nv on ern.content_id = nv.content_id
        WHERE
            ern.event_id = {} order by nbr.publish_time;
            '''.format(event_id)
        select_query_result = self.db.read_from_mysql(statement)
