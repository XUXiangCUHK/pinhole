...
class Database:
    ...
    def get_event_pool_by_id(self, event_id):
        statement = "SELECT event_id, event_top_word, event_score FROM melonfield.event_latest_info where event_id = {};".format(event_id)
        select_query_result = self.read_from_mysql(statement)
        event_pool = dict()
        for item in select_query_result:
            event_id = item[0]
