...
class load_model():
    ...
    def search_melon(self, conn, cursor, key_words):
        ...
        all_new_event_related_news, all_new_entity_related_event, all_new_event_info, all_key_words_pair, all_event_id_list, all_event_latest_news, all_delete_info, all_abstract_list, all_content_id_list = self.new_event_info_init(news_cluster)
        ...
        for item in all_content_id_list:
            event_id = item['event_id']
            content_id_list = item['content_id_list']
            keys = item['key_words']
            title = self.format_title(conn, cursor, content_id_list, keys)
            if not title:
                search_word = list()
                for pair in all_key_words_pair:
                    if event_id == pair['event_id']:
                        search_word.append(pair['search_word'])
                title = '和'.join(search_word) + '相关事件'
            search_result[event_id] = title
    ...
    def get_news_attribution_with_title(self, conn, cursor, content_id):
        statement = '''
        SELECT tf_title, nb.title
        FROM melonfield.filtered_news_info f
        JOIN streaming.news_basics nb
        ON f.content_id = nb.content_id
        WHERE f.content_id = '{}';
        '''.format(content_id)
        cursor.execute(statement)
        select_query_result = cursor.fetchall()
        ...
        title = str()
        if select_query_result:
            tf_title = Counter(json.loads(select_query_result[0][0]))
            title = select_query_result[0][1]
        return tf_title, title
    ...
    def new_event_info_init(self, news_cluster):
        ...
        for event_id, cluster_info in news_cluster.items():
            ...
            all_content_id_list.append({
                'event_id': event_id,
                'content_id_list': content_id_list,
                'key_words': key_words
            })
        return all_new_event_related_news, all_new_entity_related_event, all_new_event_info, all_key_words_pair, all_event_id_list, all_event_latest_news, all_delete_info, all_abstract_list, all_content_id_list
    
    def format_title(self, conn, cursor, content_id_list, key_words):
        pattern = r"[,|，|.|。|\?|？|!|！|:|：|;|；]"

        total_tf_title_counter = Counter()
        total_title_list = list()
        for content_id in content_id_list:
            tf_title, title = self.get_news_attribution_with_title(conn, cursor, content_id)
            total_tf_title_counter += tf_title
            total_title_list.append(title)
        
        tf_title_list = total_tf_title_counter.most_common(10)
        total_title_string = ';'.join(total_title_list).replace(' ', ';')
        split_title_list = re.split(pattern, total_title_string)

        best_x = str()
        best_score = 0
        for x in split_title_list:
            if len(x) >=5 and len(x) <=12 and key_words[1] in x:
                score = 0                    
                for tup in tf_title_list:
                    word = tup[0].split("|")[0]
                    if word in x:
                        score += tup[1]
                if score > best_score:
