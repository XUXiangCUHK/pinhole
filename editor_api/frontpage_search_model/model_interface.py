...
class load_model():
    ...
    def get_news_attribution(self, conn, cursor, content_id):
        statement = "SELECT tf_title FROM melonfield.filtered_news_info WHERE content_id = '{}';".format(content_id)
        cursor.execute(statement)
        select_query_result = cursor.fetchall()
        if select_query_result:
            tf_title = json.loads(select_query_result[0][0])
            return tf_title
        else:
            return Counter()
    ...
    def new_event_info_init(self, news_cluster):
        ...
        for event_id, cluster_info in news_cluster.items():
            ...
            self.existing_top_key_words += [x.split('|')[0] for x in json.loads(event_info[0]['event_top_word']).get('data')[:4]]
